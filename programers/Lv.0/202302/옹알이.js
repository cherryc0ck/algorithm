...
const solution = (babbling) => {
  const CORRECT_BABBLING = ["aya", "ye", "woo", "ma"];
  let answer = 0;

  babbling.forEach((x) => {
    let len = x.length;
    CORRECT_BABBLING.forEach((y) => {
      if (x.includes(y)) {
        len = x.length !== y.length ? len - y.length : 0;
        answer = len === 0 ? answer + 1 : answer;
      }
    });
  });
...
const solution2 = (babbling) => {
  ...
  const result = babbling.filter((x) => {
    let len = x.length;
    CORRECT_BABBLING.forEach((y) => {
      if (x.includes(y)) {
        len = x.length !== y.length ? len - y.length : 0;
